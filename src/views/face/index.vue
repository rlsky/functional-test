<template>
<div>
  <div>路由:{{$route.params.id}}---props:{{id}}----{{woqu}}----{{duixaing}}</div>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    console.log('face组件进入守卫:'+'去哪里-->'+to.path,',来自哪里--->'+from.path)
    next()
  },
  beforeRouteUpdate (to, from, next) {
    console.log('face组件更新守卫:'+'更新后 -->'+to.path,',更新前--->'+from.path)
    next()
  },
  beforeRouteLeave (to, from, next) {
    // ...
    console.log('face组件离开守卫:'+'去哪里-->'+to.path,',离开哪里--->'+from.path)
    next()
  },
  watch: {
    $route(to, from) {
      console.log('watch监听路由改变:'+'去哪里--->'+to.path+'来自--->'+from.path)
